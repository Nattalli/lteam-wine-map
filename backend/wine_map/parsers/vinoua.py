...
def parse_vinoua(wine_name: str) -> Optional[WineInShop]:
...
def parse_page_content(wine_name: str, page_content: str) -> Optional[WineInShop]:
    ...
    prices = soup.find_all(class_="catalogCard-price")
    if not prices:
        return None

    prices = [int(price.text.replace("грн", "").replace(" ", "")) for price in prices]
